...
export function buildDoraSystemPrompt(ctx: DoraRunContext): string {
  ...
  return [
    ...
    "- insert_after placement: think about WHERE the text belongs structurally. The anchor must be the closing text of the paragraph the insertion should follow (never mid-sentence, never inside notes/callouts/tables unless the insertion belongs there). For a new section, anchor on the end of the last paragraph of the preceding section. Separate paragraphs inside newText with a single line break, and start section insertions with their heading line.",
